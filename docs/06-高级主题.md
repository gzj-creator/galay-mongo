...
### 2. Whole-operation timeout

异步客户端的 timeout 采用 whole-operation 语义：直接在公开 awaitable 上附加 `.timeout(...)`，一个 timeout 覆盖整次 `connect` / `command` / `ping` / `pipeline` 逻辑操作。
...
auto conn = co_await client.connect(cfg).timeout(std::chrono::seconds(10));
auto res = co_await client.command("test", cmd).timeout(std::chrono::seconds(30));
```

如不显式附加 `.timeout(...)`，异步操作默认不额外启用 whole timeout。
...
异步客户端不再通过 `AsyncMongoConfig` 分别配置 send/recv timeout，而是直接对单次操作附加 `.timeout(...)`：
...
auto conn = co_await client.connect(cfg).timeout(std::chrono::seconds(10));
auto res = co_await client.command("test", cmd).timeout(std::chrono::seconds(30));
auto pipeline = co_await client.pipeline("test", std::move(commands))
...
```cpp
// 错误：whole-operation timeout 过短导致正常查询失败
AsyncMongoClient client(scheduler);
auto res = co_await client.command("test", cmd)
    .timeout(std::chrono::milliseconds(100));  // 太短！

// 正确：根据整次操作的实际耗时设置合理 timeout
auto res2 = co_await client.command("test", cmd)
    .timeout(std::chrono::seconds(30));
```
