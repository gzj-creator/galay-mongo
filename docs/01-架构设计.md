...
| `galay-mongo/sync` | `MongoSession` 同步会话 API 与同步 SCRAM 认证 |
| `galay-mongo/async` | `MongoClient` 协程 API、awaitable 状态机、异步 SCRAM 认证 |
...
| 能力 | Sync (`MongoSession`) | Async (`MongoClient`) |
|---|---|---|
| connect + hello | 支持 | 支持 |
| `command` | 支持 | 支持 |
| `ping` | 支持 | 支持 |
