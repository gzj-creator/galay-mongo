...
`AsyncMongoClient` 的异步 timeout 采用 whole-operation 语义：在 `connect`、`command`、`ping`、`pipeline` 的公开 awaitable 上按单次操作附加 `.timeout(...)`，而不是通过 `AsyncMongoConfig` 分别配置 send/recv 超时。
