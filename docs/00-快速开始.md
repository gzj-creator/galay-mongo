...
```bash
brew install cmake openssl mongodb-community
```
...
```bash
sudo apt-get update
sudo apt-get install -y cmake g++ libssl-dev mongodb
```
...
模块接口文件统一使用 `.cppm`（当前为 `galay-mongo/module/galay_mongo.cppm`）。
...
```cmake
target_link_libraries(your_target PRIVATE galay-mongo galay-kernel OpenSSL::SSL OpenSSL::Crypto)
```
